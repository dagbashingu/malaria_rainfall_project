...
```{python}
total_cases = filtered_cases["malaria_cases"].sum().item()
incidence = round(total_cases/7077181*100,1)
...
```{python}
# Create a bar chart for Malaria Cases using Plotly Express
fig_malaria_cases_combo = px.bar(
    filtered_cases,
    x="Period",
    y="malaria_cases",
    title="Malaria Cases and Test Positivity Rate (TPR)",
    labels={"Month": "Month", "Malaria_Cases": "Malaria Cases"},
    text="malaria_cases",  # Show values on bars
)

# Add a line chart for Test Positivity Rate (TPR) using Plotly Graph Objects
fig_malaria_cases_combo.add_trace(
    go.Scatter(
        x=filtered_cases["Period"],
        y=filtered_cases["Malaria Test Positivity Rate"],
        mode="lines+markers",
...
fig_malaria_cases_combo.update_layout(
    yaxis=dict(
        title="Malaria Cases",
